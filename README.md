...
WPF MVVM приложение для учёта, мониторинга и управления оборудованием организации.
...
│   ├── AuthService              Аутентификация через PostgreSQL (прямое подключение)
...
- **Pull** — загрузка устройств из ODS (автоматически при входе)
- **Commit** — сохранение в staging + трансформация в ODS
- **Push** — полный ETL-цикл через хранимые процедуры
- Подключение к БД напрямую (без прокси)
- Диагностика подключения: при ошибке показывает DNS/ping/port/SSL статус
